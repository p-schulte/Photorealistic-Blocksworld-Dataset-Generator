...
def render_scene(args,
  ...
  if args.use_gpu == 1:
    ...
    if bpy.app.version < (2, 78, 0):
      ...
      # cycles_prefs = bpy.context.user_preferences.addons['cycles'].preferences # original
      cycles_prefs = bpy.context.preferences.addons['cycles'].preferences
      cycles_prefs.compute_device_type = 'CUDA'

      # own:

      # Set the rendering engine to Cycles (GPU rendering is supported in Cycles)
      bpy.context.scene.render.engine = 'CYCLES'

      # Set the device to GPU (use CUDA for NVIDIA or OpenCL for AMD)
      bpy.context.preferences.addons['cycles'].preferences.compute_device_type = 'CUDA'  # Or 'OPENCL' for AMD
      # If you're using CUDA (for NVIDIA GPUs), this will enable the CUDA device
      bpy.context.scene.cycles.device = 'GPU'  # Use GPU rendering
